...
function showToast(msg) {
    // 1. Tạo chuỗi HTML
    const toastHTML = `<div class="my-toast">${msg}</div>`;
    // 2. Chèn vào cuối thẻ body (beforeend - Slide 48)
    document.body.insertAdjacentHTML('beforeend', toastHTML);
    // 3. Lấy phần tử vừa chèn để xử lý xóa (lastElementChild)
    const newToast = document.body.lastElementChild;
    // 4. Tự động xóa sau 3 giây
    setTimeout(() => {
...
btnAdd.addEventListener('click', function () {
    ...
    const taskHTML = `
        <li class="todo-item">
            ...
            <i class="fa-regular fa-trash-can fa-9"></i>
        </li>
    `;
    ...
    firstItem.querySelector('.fa-9').onclick = function () {
        firstItem.remove();
    };
    lastItem.querySelector('.fa-9').onclick = function () {
        lastItem.remove();
    };
});
// --- BÀI 10: COUNTDOWN ---
...
document.querySelector('#btn-10-buy').onclick = () => showToast("Mua hàng thành công!");
